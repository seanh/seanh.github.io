...
because it's more secure: when two machines share a restic repo they both have
full read write access to each other's backups. Using separate repos also
avoids locking: two machines can back up to the same repo simultaneously, but
if one machine is running certain maintenance commands like `restic prune` or
`restic check` then another machine can't back up to (or run maintenance
...
[be doing later](#accessing-another-hosts-backups).
...
RESTIC_PASSWORD="$(pass "$RESTIC_HOST"/RESTIC_PASSWORD)"

export RESTIC_HOST RESTIC_PATH RESTIC_REPOSITORY B2_ACCOUNT_ID B2_ACCOUNT_KEY RESTIC_PASSWORD
```
...
set -x RESTIC_PASSWORD (pass $RESTIC_HOST/RESTIC_PASSWORD)
```
</details>
