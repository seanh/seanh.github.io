...
This will probably work more or less the same on other operating systems as well
...
You'll need a [Backblaze B2](https://www.backblaze.com/b2/) account.
Backblaze is a great cloud storage provider, and cheap.
</div>
...
That's why my `RESTIC_REPOSITORY` environment variable contains just a
...
Now you can `source` the `env.bash` file from a bash shell and any subsequent
...
<details markdown="1">
<summary><b>Using restic interactively</b></summary>
Once you've followed the rest of the steps below to install restic and finish
creating the initial backup you can always interact with your backup by just
sourcing the env file then running restic commands:
...
Snapshots processed:   16
   Total File Count:   41622486
         Total Size:   1.350 TiB
```
</details>
...
To restore the entire contents of the latest snapshot to a `/tmp/restored` directory:
...
`latest` here means the latest snapshot *from any host*.
If you have multiple hosts backing up to the same restic repo and you want to
restore the latest snapshot from the current host either use the snapshot ID
(from `restic snapshots`) or use `--host "$(hostname)"` with `latest`:

```terminal
$ restic restore --host "$(hostname)" latest --target /tmp/restored
```
</div>

Use `--include` to restore just a single file or folder:

```terminal
$ restic restore --host "$(hostname)" latest --target /tmp/restore-work --include /foo.txt
```
...
`restic snapshots` and pass them to `restic forget`. For example:

```terminal
$ restic forget b072e1ac 65c812f2 d26a7cb7 87029ef7 203cdea7 f4c75daf c6d56f86
```
...
<div class="warning" markdown="1">
By default `restic forget` will delete snapshots from all hosts.
If you have multiple hosts backing up to the same restic repo add
...
```terminal
$ restic tag --host "$(hostname)" --add foo latest
```

You can use `restic snapshots` to see which snapshots have which tags.
