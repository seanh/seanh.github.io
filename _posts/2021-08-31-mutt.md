...
This'll bind <kbd><kbd>Ctrl</kbd> + <kbd>b</kbd></kbd> in the index or pager to open the current message in urlview.
It'll show you a list of all the URLs found in the message and you can use <kbd>Enter</kbd> to open one in a browser.
...
1. If Mutt knows how to decode the attachment to plain text then it'll open the
   attachment as plain text in the pager. This opens a pager showing _just the contents of this attachment_,
   not the whole email with all its headers, body and attachments.

2. If Mutt knows how to open the attachment in an external program then it'll do so.

3. If Mutt doesn't know how to decode or open the attachment then it'll show it in the pager as raw text.

   This usually isn't very readable.
...
<kbd>m</kbd> forces Mutt to open the attachment in an external program even if
it could render it to text itself.

<kbd>T</kbd> forces Mutt to display the attachment as raw text.
...
#### Teaching Mutt to open attachments in external programs

You can always save an attachment to file with <kbd>s</kbd> and then open it in
another program yourself.
But it's convenient to be able to open attachments in external programs
directly from Mutt.
To teach Mutt to do this you need a `mime.types` file and a `mailcap` file.
...
Once Mutt has decided what the attachment's MIME type is using the `mime.types`
file it then decides what command to use to open files of that MIME type using
the `mailcap` file. There's a `/etc/mailcap` file on Ubuntu that Mutt will find
by default and you can customize it with a `~/.mailcap` file.

You can change the list of mailcap file locations with the
[`mailcap_path`](https://mutt.org/doc/manual/#mailcap-path) setting:

    # ~/.muttrc

    set mailcap_path="~/.mailcap:~/.mutt/mailcap:/usr/share/mutt/mailcap:..."

A mailcap file maps MIME types to commands for opening files in external programs. Here's an excerpt:
...
This will make <kbd>m</kbd> open HTML attachments in Firefox,
image attachments in Eye Of Gnome
and PDF attachments in Evince.

`needsterminal` causes Mutt to wait (displaying <kbd>Press any key to continue...</kbd>)
after launching the external program. When you press any key Mutt returns to
the attachment menu. This seems to be necessary to prevent <samp>File not
found</samp> errors from Firefox.

<kbd>Enter</kbd> will also open images and PDFs in Eye Of Gnome and Evince but
will still open HTML attachments inline, showing you the HTML source code as
text:

<figure markdown="1">
![The raw text of an HTML attachment in Mutt's pager.]({{ "/assets/images/mutt-html-raw.png" | relative_url }} "The raw text of an HTML attachment in Mutt's pager.")
<figcaption markdown="1">
The raw text of an HTML attachment in Mutt's pager.
</figcaption>
</figure>

Mutt seems to "know" internally how to convert HTML to text by just showing you
the HTML source code. <kbd>m</kbd> rather than <kbd>Enter</kbd> forces Mutt to
open an HTML attachment using the command from your mailcap file.

Plain text is the same: you can add a mailcap entry for `text/plain`
attachments but you'll have to hit <kbd>m</kbd> rather than <kbd>Enter</kbd> to
use it.

#### Teaching Mutt to render attachments to text

You can also teach Mutt how to render a file type to plain text for inline
display by adding a `copiousoutput` entry to your mailcap file. For example to
use [Lynx](https://lynx.invisible-island.net/) to render HTML to text:

    # ~/.mailcap

    text/html; /usr/bin/lynx -dump -force_html %s; copiousoutput

The `copiousoutput` tells Mutt that, rather than being an interactive program for viewing the file, this is a command that converts the file to
plain text that it prints to stdout.
Now <kbd>m</kbd> will use Lynx to convert an HTML attachment to text and open
it in the pager.

#### Displaying attachments inline

If your mailcap file has a `copiousoutput` entry for a MIME type then you can
use the [`auto_view`](https://mutt.org/doc/manual/#auto-view) setting to tell
Mutt to display files of this type inline in the pager:
...
Or you can set
[`implicit_autoview`](https://mutt.org/doc/manual/#implicit-autoview) and Mutt
will render attachments for any MIME types that it finds a `copiousoutput`
entry for in the mailcap files:
...
#### Displaying attachments inline and opening them in external programs at the same time

You can teach Mutt both how to render a MIME type to text _and_ how to open it
in an external program by adding two mailcap entries: one with `copiousoutput`
and one without. The `copiousoutput` entry must always come second in the
mailcap file:
