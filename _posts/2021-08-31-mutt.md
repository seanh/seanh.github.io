...
When viewing a message with attachments the attachments appear inline in the pager
like this:

    [-- Attachment #1 --]
    [-- Type: text/plain, Encoding: 7bit, Size: 0.5K --]

This will be followed by a plain text representation of the attachment **if**
Mutt is able to render the attachment to plain text. You might even see
multiple attachments rendered inline in the pager one after another:
...
If Mutt **can't** render the attachment to text it'll display a message
where the attachment's contents would be saying
...
<kbd>v</kbd> in the index or pager opens Mutt's [attachment menu](https://mutt.org/doc/manual/#attach-menu) for the
current message. This menu shows a list of all the message's attachments and
...
<kbd>Enter</kbd> in this menu opens the selected attachment.
This'll open the attachment within Mutt if Mutt can convert it to plain text
or, if Mutt can't convert the attachment to text, it'll use your mailcap file (see below)
to decide how to open the attachment in an external program. If it can't find a
matching mailcap entry it'll display the attachment inline as raw text.

<kbd>m</kbd> forces opening the attachment in the external program specified
by your mailcap file, even if Mutt _could_ display it as text.

<kbd>T</kbd> forces displaying the attachment as raw text.

You can also save an attachment to file with <kbd>s</kbd>,
print it with <kbd>p</kbd>,
mark an attachment for deletion with <kbd>d</kbd>,
and undelete it with <kbd>u</kbd>.

To control how Mutt displays and opens attachments you use a `mime.types` file and a `mailcap` file.
...
file. The `/etc/mime.types` file on Ubuntu looks pretty comprehensive.

Once Mutt has decided what the attachment's MIME type is using the `mime.types`
file it next needs to decide what command to use to open the attachment by
using the `mailcap` file. There's a `/etc/mailcap` file on Ubuntu that Mutt
will find by default and you can customize it with a `~/.mailcap` file.
...
A mailcap file maps MIME types to opening commands. Here's an excerpt:
...
The `needsterminal` seems to be necessary to prevent <samp>File not found</samp> errors from Firefox.

You can also configure Mutt to use external programs to render attachments to
plain text and display them inline in the pager.
For example to use [ELinks](http://elinks.or.cz/) to display HTML attachments
inline first add an entry to your mailcap file:

    # ~/.mailcap

    text/html; elinks -dump -dump-color-mode 1; copiousoutput;

You have to use the `copiousoutput` flag to tell Mutt that the command is non-interactive.
Then in your muttrc add `text/html` to the [`auto_view`](https://mutt.org/doc/manual/#auto-view) setting:
...
will automatically view attachments for any MIME types that it finds a
`copiousoutput` entry for in the mailcap file. `/etc/mailcap` doesn't seem to
contain many `copiousoutput` entries so in practice this should mostly just
mean that any `copiousoutput` entries that you add to your own `~/.mailcap`
file will get used automatically:

    # ~/.muttrc

    set implicit_autoview

When mails are sent with the body in both plain text and HTML formats you can
configure which Mutt chooses to render inline with the
[`alternative_order`](https://mutt.org/doc/manual/#alternative-order) setting.
For example to prefer `text/plain` over `text/html`:
