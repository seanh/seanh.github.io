...
The mailcap tells Mutt which external commands to use to open which types of attachments.
...
opening different file types), and `apt install`'ing text mode browsers like
Lynx and ELinks will add entries for those too, as will installing text
converter programs like `html2text` and `antiword`, or viewers for images,
video, audio, and document files, etc.
...
You can use the [`auto_view`](https://mutt.org/doc/manual/#auto-view) or
[`implicit_autoview`](https://mutt.org/doc/manual/#implicit-autoview) settings
...
There has to be a `copiousoutput` entry for `text/html` in your mailcap file
for it to work: only `copiousoutput` mailcap entries can be used for inline
rendering.
...
This only works with `copiousoutput` mailcap entries.
