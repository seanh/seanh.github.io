...
* When a message first appears in a mailbox it's considered "new".
  ...
  New messages have an `N` status flag next to them in the index.
  ...
  You can toggle the current message's "new" flag on or off by pressing
  <kbd>N</kbd> in the index or pager.
  Opening a message also removes the "new" flag.
...
* If you leave a mailbox that has new messages (by quitting Mutt or going to a
  ...
  To add the "old" flag to a message that lacks it you have to press
  <kbd>w</kbd> to summon the <samp>Set flag?</samp> prompt then press
  <kbd>o</kbd>.  To remove the "old" flag press <kbd>W</kbd> (capital W) to summon the
  <samp>Clear flag?</samp> prompt then press <kbd>o</kbd>.
  Opening a message also removes the "old" flag.
...
A message can't be new and old at the same time. For example if you use
<kbd>N</kbd> to add the "new" flag to an old message it will also remove the
"old" flag.
...
so you can mark a new message as read directly from the index by pressing
<kbd>N</kbd> to remove the "new" flag, or mark an old message as read by
pressing <kbd><kbd>W</kbd> <kbd>o</kbd></kbd> to remove the "old" flag.
<kbd><kbd>Ctrl</kbd> + <kbd>r</kbd></kbd> also removes both the "new" and "old"
flags from all messages in the current thread (if the current message isn't part
of a thread it removes the flags from just the current message).
...
tell Mutt not to change new messages into old ones you leave a mailbox by
...
if you manually set the old flag with <kbd>w</kbd> (the `<set-flag>` function).
