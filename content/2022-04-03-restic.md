...
```
<details markdown="1">
  <summary>The <code>--tag "$RESTIC_TAG"</code> option</summary>

  It would be nice if restic would automatically read the `$RESTIC_TAG` envvar like it does `$RESTIC_HOST` but it doesn't.
  I opened [a feature request for it](https://github.com/restic/restic/issues/5330).
  For now we have to pass `--tag "$RESTIC_TAG"` ourselves.
</details>
...
<details markdown="1">
<summary><b>RESTIC_HOST</b> and <b>RESTIC_TAG</b></summary>
...
[Restic doesn't support reading `RESTIC_TAG` itself](https://github.com/restic/restic/issues/5330)
so we have to pass `--tag "$RESTIC_TAG"`.
</details>
